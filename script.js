...
function normalizeSupabaseUrl(value){
  ...
  // Si el usuario pega solo el Project ID, construir URL completa.
  if(url && !url.startsWith('http') && !url.includes('.supabase.co')){
    url = `https://${url}.supabase.co`;
  }

  // Si pega una URL REST, convertirla a la URL base.
...
function setSupabaseConfig(url, anonKey){
  ...
  localStorage.setItem(SUPABASE_CONFIG_KEY, JSON.stringify({url:cleanUrl, anonKey}));
}
function initSupabase(){
  const cfg = getSupabaseConfig();
  if(cfg.url) cfg.url = normalizeSupabaseUrl(cfg.url);
...
/* V35: funciones seguras para evitar errores en Admin */
function safeNumber(v){ return Number(v)||0; }
function safeDG(v){ return parseInt(String(v||'0').replace('+','')) || 0; }
function isRicardoMendezName(name){
  const n = String(name||'').toLowerCase();
  return n.includes('méndez') || n.includes('mendez') || n.includes('r. méndez') || n.includes('r. mendez');
}

function renderStandings(){
  const sel = $('standingSerieSelect');
  const tbody = $('standingRows');
  if(!sel || !tbody) return;
  const d = getData();
  const rows = [...((d.standings && d.standings[sel.value]) || [])]
    .sort((a,b)=> safeNumber(b.pts)-safeNumber(a.pts) || safeDG(b.dg)-safeDG(a.dg));

  tbody.innerHTML = rows.map((r,i)=>`
    <tr class="${i===0?'leader':''} ${isRicardoMendezName(r.team)?'rm-highlight':''}">
      <td>${i+1}</td>
      <td>${r.team||''}</td>
      <td>${r.pj??0}</td>
      <td>${r.pg??0}</td>
...
function renderAdminLists(d){
  d = d || getData();
  const fill = (id, arr, mapFn) => {
    const box = $(id);
    if(!box) return;
    box.innerHTML = (arr||[]).map((item,i)=>`
      <div class="admin-list-item">
        <span>${mapFn(item,i)}</span>
        <button type="button" onclick="deleteItem('${id}',${i})">Eliminar</button>
      </div>
    `).join('');
  };

  fill('adminNewsList', d.news, x=>`<b>${x.title||''}</b>`);
  fill('adminMediaList', d.media, x=>`<b>${x.title||''}</b> · ${x.type||''}`);
  fill('adminFixtureList', d.fixtureImages || d.fixture, x=>`<b>${x.title||x.date||''}</b> ${x.game||''}`);
  fill('adminSponsorsList', d.sponsors, x=>`<b>${x.name||''}</b>`);
  fill('adminResultsList', d.results, x=>`<b>${x.date||''}</b> ${x.match||''} ${x.score||''}`);
  fill('adminDirectivaList', d.directiva, x=>`<b>${x.role||''}</b> · ${x.name||''}`);
  fill('adminPresidentsList', d.presidents, x=>`<b>${x.name||''}</b> · ${x.period||''}`);
  fill('adminPalmaresList', d.palmares, x=>`<b>${x.year||''}</b> · ${x.title||''}`);
  fill('adminTimelineList', d.timeline, x=>`<b>${x.year||''}</b> · ${x.text||''}`);
}
...
function renderRequests(){
  ...
  if(!box) return;
  const d = getData();
  box.innerHTML = (d.requests||[]).map((r,i)=>`
    <div class="admin-list-item">
      <span><b>${r.name||''}</b> · ${r.rut||''} · ${r.phone||''} · ${r.type||''}</span>
      <button type="button" onclick="approveRequest(${i})">Aprobar</button>
      <button type="button" onclick="rejectRequest(${i})">Rechazar</button>
    </div>
  `).join('');
}

function renderCumulative(){
  const tbody = $('cumulativeRows');
  if(!tbody) return;
  const d = getData();
  const acc = {};
  Object.values(d.standings||{}).flat().forEach(r=>{
    const name = isRicardoMendezName(r.team) ? 'R. Méndez' : (r.team||'');
    if(!acc[name]) acc[name]={team:name,pj:0,pg:0,pe:0,pp:0,gf:0,gc:0,dg:0,pts:0};
    acc[name].pj += safeNumber(r.pj);
    acc[name].pg += safeNumber(r.pg);
    acc[name].pe += safeNumber(r.pe);
    acc[name].pp += safeNumber(r.pp);
    acc[name].gf += safeNumber(r.gf);
    acc[name].gc += safeNumber(r.gc);
    acc[name].dg += safeDG(r.dg);
    acc[name].pts += safeNumber(r.pts);
  });
  const rows = Object.values(acc).sort((a,b)=>b.pts-a.pts || b.dg-a.dg);
  tbody.innerHTML = rows.map((r,i)=>`
    <tr class="${i===0?'leader':''} ${r.team==='R. Méndez'?'rm-highlight':''}">
      <td>${i+1}</td><td>${r.team}</td><td>${r.pj}</td><td>${r.pg}</td><td>${r.pe}</td><td>${r.pp}</td>
      <td>${r.gf}</td><td>${r.gc}</td><td>${r.dg>0?'+':''}${r.dg}</td><td>${r.pts}</td>
    </tr>
  `).join('');
}
...
function rmSeriesRows(){
  const d = getData();
  return (typeof SERIES !== 'undefined' ? SERIES : Object.keys(d.standings||{})).map(serie=>{
    const rows = [...((d.standings && d.standings[serie]) || [])]
      .sort((a,b)=> safeNumber(b.pts)-safeNumber(a.pts) || safeDG(b.dg)-safeDG(a.dg));
    const idx = rows.findIndex(r=>isRicardoMendezName(r.team));
    const rm = idx>=0 ? rows[idx] : null;
    return {serie, pts:safeNumber(rm?.pts), pos:idx>=0?idx+1:null, dg:rm?.dg||'0'};
  });
}

function renderRanking(){
  const box = $('rankingRows');
  if(!box) return;
  const rows = rmSeriesRows().filter(r=>r.pos).sort((a,b)=>b.pts-a.pts);
  const total = rows.reduce((sum,r)=>sum+r.pts,0);
  text('rankingTotal', `${total} pts`);
  box.innerHTML = rows.map((r,i)=>`
    <div class="ranking-row"><b>${i+1}</b><span>${r.serie}<small>${r.pos}° lugar</small></span><strong>${r.pts} pts</strong></div>
  `).join('');
}
...
function renderSeriesScoreChart(){
  const chart = $('seriesScoreChart');
  if(!chart) return;
  const rows = rmSeriesRows();
  const max = Math.max(...rows.map(r=>r.pts),1);
  const total = rows.reduce((a,r)=>a+r.pts,0);
  text('chartTotalPoints', `${total} pts`);
  chart.innerHTML = rows.map(r=>`
    <div class="score-row">
      <div class="score-label">${r.serie}</div>
      <div class="score-track"><div class="score-bar" style="width:${Math.max(4,Math.round(r.pts/max*100))}%"></div></div>
      <div class="score-value">${r.pts} pts</div>
    </div>
  `).join('');
}


/* V37: función faltante renderResults y protección de render */
function renderResults(d){
  d = d || getData();
  const box = $('resultsGrid') || $('resultsList') || $('latestResults') || $('resultadosGrid');
  if(!box) return;

  const rows = d.results || [];
  if(!rows.length){
    box.innerHTML = '<article class="empty-card"><h3>Resultados pendientes</h3><p>Pronto se cargarán los últimos resultados del club.</p></article>';
    return;
  }
...
function renderNews(d){
  ...
  const grid = $('newsGrid');
  if(!grid) return;
  grid.innerHTML = (d.news||[]).map(n=>`
    <article class="news-pro">
      ${n.image ? `<img class="thumb" src="${n.image}" alt="${n.title||'Noticia'}">` : ''}
      <div>
        <span>${n.date||''}</span>
        <h3>${n.title||''}</h3>
        <p>${n.text||''}</p>
...
function renderMedia(d){
  d = d || getData();
  const grid = $('galleryGrid') || $('mediaGrid');
  if(!grid) return;
  grid.innerHTML = (d.media||[]).map(m=>{
    const isVideo = String(m.type||'').toLowerCase().includes('video') || String(m.url||'').includes('youtube') || String(m.url||'').includes('youtu.be');
    return `<article class="media-card">
      ${isVideo ? `<video src="${m.url}" controls></video>` : `<img src="${m.url}" alt="${m.title||'Media'}">`}
      <h3>${m.title||''}</h3>
    </article>`;
  }).join('');
}
...
function renderDirectiva(d){
  d = d || getData();
  const box = $('directivaGrid');
  if(!box) return;
  box.innerHTML = (d.directiva||[]).map(x=>`
    <article><h3>${x.role||''}</h3><p>${x.name||''}</p></article>
  `).join('');
}

function renderPalmares(d){
  d = d || getData();
  const box = $('palmaresGrid');
  if(!box) return;
  box.innerHTML = (d.palmares||[]).map(x=>`
    <article><b>${x.year||''}</b><p>${x.title||''}</p></article>
  `).join('');
}

function renderTimeline(d){
  d = d || getData();
  const box = $('timelineGrid') || $('timelineList');
  if(!box) return;
  box.innerHTML = (d.timeline||[]).map(x=>`
    <article><b>${x.year||''}</b><p>${x.text||''}</p></article>
  `).join('');
}
...
function setup(){$('standingSerieSelect')?.addEventListener('change',renderStandings); document.querySelectorAll('.serie-pill').forEach(btn=>btn.addEventListener('click',()=>{$('standingSerieSelect').value=btn.dataset.serie; document.querySelector('#posiciones')?.scrollIntoView({behavior:'smooth'}); renderStandings()}));
 ...
 $('syncToSupabase')?.addEventListener('click',async()=>{
   ...
   if(!ok) return alert('Supabase no está conectado. Primero presiona GUARDAR CONEXIÓN SUPABASE y verifica URL + Publishable Key.');
